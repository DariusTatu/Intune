...
Try {
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        ## Check if Teamviewer FullClient is already installed 
        If(Test-Path "C:\Program Files (x86)\TeamViewer\TeamViewer.exe")
        {
            Write-Log "Teamviewer Host was found. Uninstalling..."
            Execute-MSI -Action Uninstall  -Path "{C38893D8-7FEE-41DF-9963-2CCE4F9C61DC}" -Parameters "/qn"
            Write-Log "Succesfully uninstalled Teamviewer Host! Continuing installation of Teamviewer FullClient AutoUpdate..."
        }
        else
        {
            Write-Log "Teamviewer Host was not found. Skipping uninstall..."
        }
        ...
        Execute-MSI -Action "Install" -Path "$dirFiles\TeamViewer_Full.msi" -Parameters "/quiet /norestart" 

        ## Enable automatic updates
        Try
        {
            Write-Log "Trying to add registry keys for enabling automatic updates..."
            New-ItemProperty -Path "HKLM:\SOFTWARE\TeamViewer" -Name "AutoUpdateMode" -Value "0"  -PropertyType "DWORD"
            New-ItemProperty -Path "HKLM:\SOFTWARE\TeamViewer" -Name "UpdateCheckInterval" -Value "1"  -PropertyType "DWORD"
            
            Write-Log "Succesfully set registry keys for enabling automatic updates!"            
        }
        catch
        {
            Write-Log "There was a problem adding necessary registrykeys for enabling automatic updates. $_"
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Execute-MSI -Action Uninstall -Path "{2C8A527C-5094-4CDA-A734-C204A1FDC907}"
