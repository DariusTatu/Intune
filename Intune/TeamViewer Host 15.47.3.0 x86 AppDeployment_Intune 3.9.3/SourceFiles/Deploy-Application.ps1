...
Try {
    ...
    [String]$appName = 'HostClient'
    [String]$appVersion = '15.47.3.0'
    [String]$appArch = 'x86'
    [String]$appLang = 'MUI'
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.0'
    [String]$appScriptDate = '09/11/2023'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        ## Check if Teamviewer is installed if yes uninstall it
        If(Test-Path "C:\Program Files (x86)\TeamViewer\Uninstall.exe")
        {
            Try
            {
                Write-Log "Old Teamviewer Host version detected. Uninstalling.."
                Stop-Process -Name "TeamViewer" -Force
                Execute-Process -Path "${env:ProgramFiles(x86)}\TeamViewer\uninstall.exe" -Parameters "/S"
                Write-Log "Succesfully uninstalled Teamviewer Host! Continuing with installation..."
            }
            catch
            {
                Write-Log "There was a problem uninstalling old Teamviewer Host version. $_"
            }
        }
        else
        {
            Write-Log "No old Teamviewer Host version was found. Checking for MSI Installation..."
        }

        If(Test-Path "C:\Program Files (x86)\TeamViewer")
        {
            Try
            {
                Write-Log "Old MSI Teamviewer Host version detected. Uninstalling.."
                Stop-Process -Name "TeamViewer" -Force
                Execute-MSI -Action Uninstall -Path "{79D72D2D-E3BE-440F-96DD-6C8FC45C2E0C}" 
                Write-Log "Succesfully uninstalled MSI Teamviewer Host! Continuing with installation..."
            }
            catch
            {
                Write-Log "There was a problem uninstalling old MSI Teamviewer Host version. $_"
            }
        }
        else
        {
            Write-Log "No old MSI Teamviewer Host version was found. Continuing the installation.."
        }
        ...
        ## Installation Twingate
        
        Execute-MSI -Action "Install" -Path "$dirFiles\TeamViewer_Host.msi" -Parameters "/quiet /norestart" 
        
        #SETTINGSFILE='$dirFiles\TeamViewer_Settings.tvopt
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Execute-MSI -Action Uninstall -Path "{C38893D8-7FEE-41DF-9963-2CCE4F9C61DC}"
