...
Try {
    ...
    [String]$appName = 'SAP IBP add-in for Microsoft Excel'
    [String]$appVersion = '23.11.2.0'
    [String]$appArch = 'x64'
    [String]$appLang = 'MUI'
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.0'
    [String]$appScriptDate = '30/10/2023'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Remove-MSIApplications -Name 'SAP IBP, add-in for Microsoft Excel'

        ##Check if Microsoft EdgeWebView is installed

        $WebView2 = Get-ChildItem -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall","HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall" | Get-ItemProperty | Where-Object {$_.DisplayName -match 'WebView2' } | Select-Object -Property DisplayName, DisplayVersion

        $WebView2.DisplayVersion

        ($WebView2.DisplayVersion -gt '1')
        ...
        ## <Perform Installation tasks here>
            ...
            If($WebView2.DisplayVersion -lt '1')
        {
            Write-Log "Microsoft EdgeWebView is not installed on this client. Installing..."
            Execute-Process -Path "$dirSupportFiles\MicrosoftEdgeWebView2RuntimeInstallerX64.exe" -Parameters '/silent /install'
        }
        
        Else{($WebView2.DisplayVersion -gt '1')}
        {
            Write-Log "Microsoft EdgeWebView is already installed on this client. Exiting installation..."
        }
       

        Execute-Process -Path "$dirSupportFiles\vc_redist.x64.exe" -Parameters '/norestart /quiet' -ContinueOnError $true

        Execute-Process -Path "$dirSupportFiles\windowsdesktop-runtime-6.0.24-win-x64.exe" -Parameters '/norestart /quiet' -ContinueOnError $true
    
        
        
        ## Installation SAP IBP add-in for Microsoft Excel

        Execute-MSI -Action 'Install' -Path "IBP_Add-in_NET5_x64.msi" -parameters "/norestart /quiet"
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Execute-MSI -Action 'Uninstall' -Path "IBP_Add-in_NET5_x64.msi" -parameters "/QN"
