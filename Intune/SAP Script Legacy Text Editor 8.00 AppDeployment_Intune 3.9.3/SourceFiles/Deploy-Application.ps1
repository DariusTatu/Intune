...
Try {
    ...
    [String]$appName = 'SAP Script Legacy Text Editor'
    [String]$appVersion = '8.00 P0'
    [String]$appArch = 'x64'
    [String]$appLang = ''
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.0'
    [String]$appScriptDate = '14/09/2023'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        $Uninstalled = 1 ## set variable for skipping Uninstall
        $registryPath = "HKLM\SOFTWARE\MHP-Software\SAP SAP Script Legacy Text Editor 7.70 P0 x86 "

        If(Test-Path $registryPath)
            {
                $Uninstalled = 1
            }
        If(-Not($Uninstalled -eq 0))

            {
            Write-Log "SAP Script Legacy Text Editor 7.70 P0 x86  is installed, starting uninstall..."
            Execute-Process -Path "C:\Program Files (x86)\SAP\SapSetup\Setup\NwSapSetup.exe" -Parameters "/Uninstall /Product='SCRIPTED' /silent /NoDlg /norestart" -IgnoreExitCodes '129' 
            Remove-RegistryKey $registryPath
            }
        If($Uninstalled -eq 1)
            {
            Write-Log "SAP Script Legacy Text Editor 7.70 P0 x86 is already uninstalled on this client."
            }
		...
		## Installation SAP Script Legacy Text Editor 8.00 P0
        
        Execute-Process -Path "$dirFiles\Setup\NwSapSetup.exe" -Parameters "/package='SAPSLTE80' /silent /NoDlg /norestart" -IgnoreExitCodes '129'
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Execute-Process -Path "$envProgramFiles\SAP\SAPsetup\Setup\NwSapSetup.exe" -Parameters "/Uninstall /package='SAPSLTE80' /silent /NoDlg /IgnoreMissingProducts /norestart" -IgnoreExitCodes '129'
