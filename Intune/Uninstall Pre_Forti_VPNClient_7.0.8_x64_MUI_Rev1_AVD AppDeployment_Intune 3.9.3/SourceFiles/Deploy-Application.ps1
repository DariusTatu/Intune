...
Try {
    ...
    [String]$appVendor = 'Forti'
    [String]$appName = 'VPNClient'
    [String]$appVersion = '7.0.8'
    [String]$appArch = 'x64'
    [String]$appLang = 'AVD'
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.0'
    [String]$appScriptDate = '28/06/2023'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        If(Test-Path "C:\Program Files\Fortinet\FortiClient\FortiClient.exe")
        {
            Try
            {
                Write-Log "Old FortiClient version detected. Uninstalling.."
                Start-Process -FilePath "C:\Program Files\Fortinet\FortiClient\FortiESNAC.exe" -ArgumentList "-u" -Wait
                Start-Process -FilePath "MsiExec.exe" -ArgumentList "/x {12C8C9B7-3C65-4B0E-9DCF-1A1451BABC75} /qn /norestart" -Wait
                Write-Log "Succesfully uninstalled FortiClient! Reboot required to continue with installation..."
                Write-Log "Delete registry keys for FortiClient..."
                Remove-Item $RegkeyApplicationFullName -recurse
                Remove-Item -Path "HKLM:\SOFTWARE\MHP-Software\Pre_Forti VPNClient 7.0.8 x64 AVD" -recurse
                
            }
            catch
            {
                Write-Log "There was a problem uninstalling old FortiClient version. $_"
            }
        }
